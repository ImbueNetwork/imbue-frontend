...
const MilestoneVoteBox = (props: MilestoneVoteBoxProps) => {
  ...
  const firstPendingMilestone =
    props?.firstPendingMilestone >= 0 &&
    props.firstPendingMilestone < project?.milestones?.length
      ? props?.firstPendingMilestone
      : project?.milestones?.length - 1;

  const currentMilestoneName = project?.milestones?.length
    ? project?.milestones?.[firstPendingMilestone]?.name || ''
    : '';
    ...
    <div>
      {loading ? (
        ...
        <div className='flex justify-between text-black'>
          ...
          <p>
            {firstPendingMilestone + 1}/{project?.milestones?.length}
          </p>
        </div>
      )}
        ...
        <div className='flex mt-7 justify-between gap-2'>
          {loading ? (
            ...
            <div
              ...
              onClick={() =>
                !project.approvers && props?.setOpenVotingList(true)
              }
            >
              <AvatarGroup className='justify-end' max={3}>
                {votes?.no?.map((v, index) => (
                  <Avatar
                    ...
                    key={index}
                    alt='Remy Sharp'
                    src={v.profile_photo || '/profile-image.png'}
                  />
                ))}
              </AvatarGroup>
