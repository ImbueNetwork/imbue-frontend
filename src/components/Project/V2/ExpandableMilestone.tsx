...
import SuccessModal from '@/components/ReviewModal/SuccessModal';
import SuccessRefundModal from '@/components/RefundModal/SuccessRefundModal';
    ...
    try {
      ...
      while (true) {
        if (result.status || result.txError) {
          if (result.status) {
            const resp = await updateProjectVotingState(
              Number(project.id),
              true
            );
            if (resp) {
  ...
  const withdraw = async (account: WalletAccount) => {
    ...
    try {
      ...
      while (true) {
        if (result.status || result.txError) {
          console.log(
            '🚀 ~ file: ExpandableMilestone.tsx:226 ~ withdraw ~ result:',
            result
          );
          if (result.status) {
            const haveAllMilestonesBeenApproved = projectMilestones
              ...
              .every(Boolean);
            console.log(
              '🚀 ~ file: ExpandableMilestone.tsx:229 ~ withdraw ~ haveAllMilestonesBeenApproved:',
              haveAllMilestonesBeenApproved
            );
      ...
      <Accordion className='shadow-none mt-5 before:h-0 !rounded-xl py-5'>
        <AccordionSummary aria-controls='panel1a-content' id='panel1a-header'>
          <Typography className='grid grid-cols-12 gap-5 w-full'>
            ...
            project.project_in_milestone_voting ? (
              <p
                className={`px-4 py-1.5 rounded-full col-start-11 justify-self-start col-end-13 ml-auto ${
          ...
          <Typography className='px-2'>
            <div>
              ...
              <p className='mt-5'>
                ...
                pushing out our first MVP based on our product roadmap; for this
                we would hired talents on imbue including a designer, a UX
                writer, a Full Stack Developer and a Project Manager. We
                estimated our MVP Design and Development to involve Key steps,
                Hiring, Onboarding and task delegation; and at the end of this
                Milestone we now have a live Website for our Waitlist. You can
                find a link to the live website
              </p>
              <p className='text-black mt-10 font-semibold text-lg'>
                Project Attachments
              </p>
              ...
              <div className='w-full mt-7 flex'>
                ...
                {(isApplicant || (projectType === 'grant' && isProjectOwner)) &&
                  milestone.milestone_index ==
                    project.first_pending_milestone &&
                  !project.project_in_milestone_voting &&
                  !milestone?.is_approved && (
                    <button
