...
const ExpandableDropDowns = (props: ExpandableDropDownsProps) => {
  ...
  const handleVoteOnMilestone = async (
  ...
  const withdraw = async (account: WalletAccount) => {
    ...
    console.log("***** milestone key in view is");
    console.log(milestoneKeyInView);
    setLoading(true);
    const imbueApi = await initImbueAPIInfo();
    ...
    <div className='mb-2 relative bg-white px-5 border border-white rounded-2xl lg:px-12 max-width-750px:!pb-[30px]'>
      ...
      >
        ...
        <div className='flex flex-row items-center max-width-750px:w-full max-width-750px:justify-between'>
          {milestonLoadingTitle ? (
            <p className='mr-5 text-content-primary text-sm'>
              {milestonLoadingTitle}
            </p>
          ) : (
            <>
          ...
          projectInMilestoneVoting && (
            ...
            >
              <button
                ...
                onClick={() =>
                  canVote && handleVoting(milestone.milestone_index)
                }
              >
                Vote
              </button>
            ...
            >
              <button
                className={`primary-btn in-dark w-button mt-3 ${
                  !balance && '!bg-gray-300 !text-gray-400 !cursor-not-allowed'
                }`}
                data-testid='next-button'
                onClick={() =>
                  balance && handleSubmitMilestone(milestone.milestone_index)
                }
              >
                Submit
              </button>
