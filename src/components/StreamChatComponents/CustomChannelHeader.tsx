...
export function CustomChannelHeader(props: any) {
  ...
  const membersToArray = Object.keys(members)

  const membersCount = membersToArray?.length;
  let chatTitle = 'Not Found';
  let username = 'Not Found';
  let profile_photo = null;

  membersToArray?.forEach(function (key) {
    if (membersCount === 2 && key !== client.userID) {
      ...
      profile_photo = members[key]?.user?.profile_photo
    }
  });
