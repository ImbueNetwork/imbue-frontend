import { Badge, Box, CircularProgress, Modal, TextField, Tooltip } from '@mui/material';
      ...
      <div className="grid grid-cols-2 justify-center md:grid-cols-3 w-full mt-2">
              ...
              <Tooltip title={client?.website} placement="top" arrow>
                <div className="flex items-center gap-3 w-fit">
                  <Badge onClick={(e) => removeClient(e, client?.logo)} className="client-badge" color="error" overlap="circular" badgeContent="-" invisible={!isEditMode}>
                    <div>
                      <Image className="rounded-lg h-12 w-12 object-cover" height={100} width={100} src={client?.logo} alt={client?.name} />
                    </div>
                  </Badge>
        ...
        >
          <Box sx={style}>
              ...
              newClient?.logo
                ? (
                  ...
                  >
                    ...
                    <Image className='w-full h-full object-cover' src={newClient.logo} width={180} height={180} alt='' />
                  </label>
                )
                : (
                  <label
            ...
            <TextField onChange={(e) => handleNewClientData(e)} id="outlined-basic" label="Client Name" variant="outlined" color='secondary' name='name' />
            <TextField onChange={(e) => handleNewClientData(e)} id="outlined-basic" label="Website Link" variant="outlined" color='secondary' name='website' />
