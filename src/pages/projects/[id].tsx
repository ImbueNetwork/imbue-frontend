...
function Project() {
  ...
  // get last approved milestone
  const lastApprovedMilstone = async () => {
    setLoading(true);
    const imbueApi = await initImbueAPIInfo();
    const user: User | any = await utils.getCurrentUser();
    const chainService = new ChainService(imbueApi, user);
    const result = await chainService.findLastApprovedMilestone(
      onChainProject?.milestones
    );
    console.log({ result });
    // while (true) {
    //   if (result.status || result.txError) {
